...
function buildImageAlt(car) {
  const base = buildCarName(car) || 'Auto seminuevo';
  const parts = [
...
export default function CarCard({
  ...
  return (
    <MotionArticle
      ...
      className="car-card relative z-10 flex h-full flex-col overflow-hidden rounded-xl border border-white/10 bg-gray-900/50 shadow-lg transition-transform hover:-translate-y-1 hover:shadow-amber-500/20"
    >
      <Link
        to={detailUrl}
      ...
      >
        <div className="relative flex aspect-square w-full items-center justify-center overflow-hidden bg-gray-800">
          ...
          />
          <div className="pointer-events-none absolute inset-0 z-10 bg-gradient-to-t from-gray-900/80 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
        </div>
      </Link>

      <div className="relative z-20 flex flex-grow flex-col bg-gray-900/50 p-6 text-white backdrop-blur-sm">
        ...
        {(car.propietarios || car.origen) && (
          <div className="mb-6 grid grid-cols-1 gap-3 border-b border-white/20 pb-6 text-sm font-medium text-gray-300">
            {car.propietarios && (
              <div className="flex items-center space-x-2">
                <Users className="h-4 w-4 flex-shrink-0 text-amber-500" />
                <span className="truncate">{car.propietarios}</span>
              </div>
            )}
            ...
            {car.origen && (
              <div className="flex items-center space-x-2">
                ...
                <span className="truncate">{car.origen}</span>
              </div>
            )}
          </div>
        )}

        <div className="mb-6 flex items-center space-x-2 text-sm font-medium text-gray-300">
          <ShieldCheck className="warranty-shield h-5 w-5 flex-shrink-0 text-amber-500" />
          <span>{car.warranty || 'Garantía incluida'}</span>
        </div>
        ...
        <div className="mt-auto border-t border-white/20 pt-6">
          <div className="mb-4 flex flex-col items-center gap-3">
            ...
            <Link
              to={detailUrl}
              className="text-sm font-semibold text-amber-400 underline-offset-4 hover:text-amber-300 hover:underline"
