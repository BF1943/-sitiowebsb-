// Configuration for the static HTML pre-renderer
export const routes = [
  {
    path: '/',
    title: 'Seminuevos Ensenada | Autos Nacionales con Garantía',
    description:
      'Compra autos seminuevos nacionales en Ensenada con póliza de garantía mecánica de 12 meses, financiamiento y atención directa.',
  },
  {
    path: '/inventario',
    title: 'Autos Seminuevos en Ensenada | Seminuevos Baja',
    description:
      'Explora autos seminuevos y usados en venta en Ensenada. Unidades nacionales con financiamiento y póliza de garantía mecánica de 12 meses.',
  },
  {
    path: '/vender',
    title: 'Cotización de Mi Auto | Vende tu Auto en Ensenada',
    description:
      'Vende tu auto usado en Ensenada con cotización profesional, oferta clara y opción de pago inmediato en Seminuevos Baja.',
  },
  {
    path: '/consigna',
    title: 'Consigna tu Auto en Ensenada | Precio Justo de Mercado',
    description:
      'Consigna tu auto en Ensenada y véndelo a un precio justo de mercado con apoyo profesional y atención a compradores reales.',
  },
  {
    path: '/consigna-desde-tijuana',
    title: 'Consigna tu Auto desde Tijuana | Mejor Precio | Seminuevos Baja',
    description:
      'Si en Tijuana te ofrecen menos de lo esperado, consigna tu auto en Ensenada con Seminuevos Baja y obtén una mejor oportunidad de venta.',
  },
  {
    path: '/financiamiento',
    title: 'Autos a Crédito en Ensenada | Financiamiento',
    description:
      'Consulta autos seminuevos a crédito en Ensenada. Te orientamos según vehículo, enganche, plazo y perfil del comprador.',
  },
  {
    path: '/gana-dinero',
    title: 'Programa de Referidos | Gana Dinero | Seminuevos Baja',
    description:
      'Gana dinero refiriendo compradores y vendedores de autos en Ensenada. Únete al programa de referidos de Seminuevos Baja.',
  },
  {
    path: '/quienes-somos',
    title: 'Agencia de Autos en Ensenada | Seminuevos Baja',
    description:
      'Conoce Seminuevos Baja, agencia de autos en Ensenada enfocada en compra, venta y consignación de autos nacionales con atención personalizada.',
  },
  {
    path: '/blog',
    title: 'Blog de Autos Seminuevos en Ensenada | Seminuevos Baja',
    description:
      'Guías y artículos sobre compra, venta, consignación, financiamiento y seguridad para autos seminuevos en Ensenada y Baja California.',
  },
  {
    path: '/contacto',
    title: 'Contacto | Autos Seminuevos en Ensenada | Seminuevos Baja',
    description:
      'Contáctanos para agendar una cita, resolver dudas sobre inventario, financiamiento, compra, venta o consignación de autos en Ensenada.',
  },
  {
    path: '/terminos',
    title: 'Términos y Condiciones | Seminuevos Baja',
    description:
      'Consulta los términos y condiciones de Seminuevos Baja sobre el uso del sitio web, formularios, contenido y servicios.',
  },

  // Blog posts
  {
    path: '/blog/mejores-seminuevos-tijuana-ensenada-2025',
    title: 'Los Mejores Autos Seminuevos en Tijuana y Ensenada 2025 | Seminuevos Baja',
    description:
      'Descubre los mejores autos seminuevos disponibles en Tijuana y Ensenada para 2025. Guía con precios, características y recomendaciones.',
  },
  {
    path: '/blog/guia-seminuevos-ensenada',
    title: 'Cómo Comprar un Seminuevo en Ensenada: Agencia vs Tianguis | Seminuevos Baja',
    description:
      '¿Comprar seminuevo en agencia o en opciones informales en Ensenada? Comparamos riesgos, contexto de compra y criterios para decidir mejor.',
  },
  {
    path: '/blog/comprar-auto-seminuevo-sin-ser-estafado',
    title: 'Cómo Comprar un Auto Seminuevo Sin Ser Estafado | Seminuevos Baja',
    description:
      'Aprende a identificar fraudes y protegerte al comprar un auto seminuevo en Ensenada. Consejos prácticos para comprar con más seguridad.',
  },
  {
    path: '/blog/autos-chocolate-vs-nacionales',
    title: 'Autos Chocolate vs Nacionales en Baja California | Seminuevos Baja',
    description:
      '¿Conviene más un auto chocolate o nacional en Baja California? Analizamos costos, riesgos legales y valor a largo plazo para ayudarte a decidir.',
  },
  {
    path: '/blog/ruta-del-vino-valle-de-guadalupe-auto-ideal',
    title: 'El Auto Ideal para la Ruta del Vino en Valle de Guadalupe | Seminuevos Baja',
    description:
      'Descubre qué auto seminuevo es ideal para recorrer la Ruta del Vino en Valle de Guadalupe. SUVs, sedanes y más opciones analizadas.',
  },
  {
    path: '/blog/autos-por-decreto-seguro-punto-ciego',
    title: 'Autos por Decreto: Seguros, Riesgos y Punto Ciego | Seminuevos Baja',
    description:
      'Lo que debes saber sobre los autos por decreto en Baja California: seguros, riesgos, trámites y puntos ciegos antes de comprar.',
  },
  {
    path: '/blog/yardas-carros-ensenada',
    title: 'Yardas de Carros en Ensenada vs Agencias de Seminuevos | Seminuevos Baja',
    description:
      'Comparativa entre comprar en yardas de carros o en una agencia de seminuevos en Ensenada. Ventajas, desventajas y recomendaciones.',
  },
  {
    path: '/blog/venta-carros-ensenada',
    title: 'Venta de Carros en Ensenada | Autos con Garantía | Seminuevos Baja',
    description:
      'Guía para comprar autos en Ensenada con respaldo, revisión legal y opciones de financiamiento. Conoce qué revisar antes de elegir un seminuevo.',
  },
  {
    path: '/blog/seminuevos',
    title: '¿Conviene Comprar un Seminuevo en Ensenada? Qué Revisar Antes | Seminuevos Baja',
    description:
      'Descubre cuándo conviene comprar un seminuevo en Ensenada, qué revisar antes de elegir y por qué un auto revisado puede ser mejor compra que uno nuevo básico.',
  },
  {
    path: '/blog/vender-auto-financiado',
    title: 'Cómo Vender un Auto Financiado en México | Seminuevos Baja',
    description:
      'Aprende paso a paso cómo vender tu auto aunque aún tenga financiamiento activo en México. Opciones legales y recomendaciones prácticas.',
  },
  {
    path: '/blog/consignacion-autos-segura',
    title: 'Consignación de Autos Segura en Ensenada | Seminuevos Baja',
    description:
      'Descubre cómo funciona la consignación de autos en Ensenada y por qué puede ser una alternativa más segura y conveniente para vender tu vehículo.',
  },
];